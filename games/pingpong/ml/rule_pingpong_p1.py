# -*- coding: utf-8 -*-
"""
Created on Thu Apr  8 15:53:09 2021
    ...
    def update(self, scene_info):
        ...
        if not self.ball_served:
            ...
            if y_speed <0 and y_ball >260 and judge:
                ...
                pred = x_ball + times*x_speed
                pred_blo = block + times*block_speed
                if pred >=195:
                    wall=1
                    pred = 390-pred + abs(y_speed) - (y_ball-260)%abs(y_speed)
                elif pred < 0:
                    wall = 1
                    pred = pred*(-1) - (abs(y_speed) - (x_ball-260)%abs(y_speed))
                if pred_blo >=170:
                    pred_blo = 340 - pred_blo
                elif pred_blo <0:
                    pred_blo=pred_blo*(-1)
                if pred_blo-5  <= pred and pred<= pred_blo + 30:
                    dest = pred + 155*slope*wall
            elif y_speed > 0 :
